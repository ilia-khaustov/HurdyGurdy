    ...
    exports.addClient = function(client) {
        if (client.socket) {
            ...
            client.socket.on('send_access_token', function(access_token){
                client.access_token = access_token;                
                console.log('access_token:' + access_token);
                if(access_token){
                    var vk = vkontakte(access_token);                
                    vk('users.get', { fields: 'uid,first_name,photo' }, function (error, data) {
                        var user = data[0];                    
                        client.socket.emit('client_user_data',user);
                        client.user = user;
                        console.log(user);
                    });
                }
